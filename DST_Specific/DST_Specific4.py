...
st.markdown("""
    <style>
        ...
        /* Optional: Adjust padding on the tab buttons if needed */
        .stTabs [role="tab"] {
...
LOCAL_KOPPEN_PATH = os.path.join(LOCAL_DATA_DIR, "Koppen/1991-2020/koppen_geiger_0p1.tif")

items = [
    {"name": "Demo site 1-A", "address": "Lattenbach Valley, Austria", "icon_url": os.path.join(LOCAL_IMAGE_DIR, "logo1.jpg"), "github_key": "demo1a", "coordinate": [47.148472, 10.499805]},
    {"name": "Demo site 1-B", "address": "Brunntal, Austria", "icon_url": os.path.join(LOCAL_IMAGE_DIR, "logo2.jpg"), "github_key": "demo1b", "coordinate": [47.625027, 15.052111]},
    {"name": "Demo site 2", "address": "Brasov City, Romania", "icon_url": os.path.join(LOCAL_IMAGE_DIR, "logo3.jpg"), "github_key": "demo2", "coordinate": [45.647078, 25.593030]},
    {"name": "Demo site 3", "address": "Slovenia", "icon_url": os.path.join(LOCAL_IMAGE_DIR, "logo4.jpg"), "github_key": "demo3", "coordinate": [46.0345, 14.461]},
    {"name": "Demo site 4", "address": "Zvolen, Slovakia", "icon_url": os.path.join(LOCAL_IMAGE_DIR, "logo5.jpg"), "github_key": "demo4", "coordinate": [48.5707, 19.1462]},
    {"name": "Demo site 5", "address": "Globocica, Macedonia", "icon_url": os.path.join(LOCAL_IMAGE_DIR, "logo6.png"), "github_key": "demo5", "coordinate": [48.5647, 19.114430]},
]
BACKGROUND_IMAGE_PATH = os.path.join(LOCAL_IMAGE_DIR, "main_logo.png")

@st.cache_data(ttl=3600)
def local_base64_image(path: str) -> str | None:
    try:
...
@st.cache_data(ttl=3600)
def local_read_excel(path: str) -> pd.DataFrame | None:
    try:
...
def list_level1_files(github_key: str):
    dir_path = os.path.join(LOCAL_DATA_DIR, "texts", github_key, "level1")
    if not os.path.exists(dir_path):
        return []
    return [{"name": name, "download_url": os.path.join(dir_path, name)} for name in os.listdir(dir_path) if os.path.isfile(os.path.join(dir_path, name))]

@st.cache_data(ttl=600)
def get_sorted_txt_files(github_key: str):
    dir_path = os.path.join(LOCAL_DATA_DIR, "texts", github_key)
    if not os.path.exists(dir_path):
        return []
    txts = [{"name": name, "download_url": os.path.join(dir_path, name)} for name in os.listdir(dir_path) if name.endswith(".txt")]
    def keyfn(f):
...
def get_kpis_excel(github_key: str) -> pd.DataFrame | None:
    path = os.path.join(LOCAL_DATA_DIR, "texts", github_key, "level1", "1KPIs.xlsx")
    return local_read_excel(path)

@st.cache_data(ttl=600)
def get_el_excel(github_key: str) -> pd.DataFrame | None:
    path = os.path.join(LOCAL_DATA_DIR, "texts", github_key, "level1", "2el.xlsx")
    return local_read_excel(path)

@st.cache_data(ttl=600)
def get_interpretation_text(github_key: str) -> str | None:
    path = os.path.join(LOCAL_DATA_DIR, "texts", github_key, "level1", "interpretation.txt")
    try:
...
def get_climate_report_text(github_key: str) -> str | None:
    path = os.path.join(LOCAL_DATA_DIR, "texts", github_key, "climate", "climate_report.txt")
    try:
        return local_text(path)
    except Exception:
        return None

bg_b64 = local_base64_image(BACKGROUND_IMAGE_PATH)
if bg_b64:
...
for it in items:
    b64 = local_base64_image(it["icon_url"])
    if b64:
...
with tab1:
    ...
    fig = quick_koppen_map(LOCAL_KOPPEN_PATH, map_center[0], map_center[1])
    if fig is not None:
        st.pyplot(fig)
    else:
...
# Content for Tab 2
with tab2:
    st.header("Information tables")
    try:
        files_json = list_level1_files(selected_item["github_key"])
        xlsx_files = [f for f in files_json if f.get("name", "").endswith(".xlsx")]
        def order_fn(name):
        ...
        for f in xlsx_files:
            name = f["name"]
            modified = re.sub(r'^\d+_', '', name).replace('.xlsx', '')  # Fixed potential bug by removing name[1:]
