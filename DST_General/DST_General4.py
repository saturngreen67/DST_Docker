...
KOPPEN_TIFF_PATH = "/app/data/koppen_geiger_0p1.tif"
...
def generate_koppen_map_plot(lat, lon, zoom_range=1.0):
    ...
    try:
        with rasterio.open(KOPPEN_TIFF_PATH) as src:
            data = src.read(1)
        ...
        return f"Error reading Köppen TIFF file from {KOPPEN_TIFF_PATH}: {e}. Make sure the file exists.", None
...
with st.sidebar:
    ...
    st.image('/app/data/main_logo.png', width=300)
